...
import "codemirror/mode/php/php"; // Use the 'php' mode for PHP
...
function PHPEditor() {
  ...
  const handleCompile = () => {
    ...
    const formData = {
      language_id: 68, // PHP language ID for Judge0 API
      ...
      <Container fluid>
        <Row style={{ justifyContent: "center", padding: "25px" }}>
          <Col md={6} className="text-center">
            <h3 className="text-center">PHP Editor</h3>
            <CodeMirror
              value={code}
              options={{
            ...
            <Button onClick={handleCompile} disabled={processing}>
              {processing ? "Compiling..." : "Compile and Run PHP Code"}
            </Button>
          </Col>
          <Col md={6} style={{ paddingTop: "35px" }}>
            <h3 className="text-center">PHP Output</h3>
