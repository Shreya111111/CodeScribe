...
import "codemirror/mode/shell/shell"; // Use the 'shell' mode for Bash
...
function BashEditor() {
  ...
  const handleCompile = () => {
    ...
    const formData = {
      language_id: 46, // Bash language ID for Judge0 API
      ...
      <Container fluid>
        <Row style={{ justifyContent: "center", padding: "25px" }}>
          <Col md={6} className="text-center">
            <h3 className="text-center">Bash Editor</h3>
            <CodeMirror
              value={code}
              options={{
                mode: "text/x-sh",
            ...
            <Button onClick={handleCompile} disabled={processing}>
              {processing ? "Compiling..." : "Compile and Run Bash Script"}
            </Button>
          </Col>
          <Col md={6} style={{ paddingTop: "35px" }}>
            <h3 className="text-center">Bash Output</h3>
