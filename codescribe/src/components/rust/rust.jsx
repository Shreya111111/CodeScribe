...
import "codemirror/mode/rust/rust"; // Use the 'rust' mode for Rust
...
function RustEditor() {
  const initialCode = `// Welcome to Rust Editor\n\nfn main() {\n\tprintln!("Hello, World!");\n}`;
  ...
  const handleCompile = () => {
    ...
    const formData = {
      language_id: 73, // Rust language ID for Judge0 API
      ...
      <Container fluid>
        <Row style={{ justifyContent: "center", padding: "25px" }}>
          <Col md={6} className="text-center">
            <h3 className="text-center">Rust Editor</h3>
            <CodeMirror
              value={code}
              options={{
                mode: "text/x-rust",
            ...
            <Button onClick={handleCompile} disabled={processing}>
              {processing ? "Compiling..." : "Compile and Run Rust Code"}
            </Button>
          </Col>
          <Col md={6} style={{ paddingTop: "35px" }}>
            <h3 className="text-center">Rust Output</h3>
